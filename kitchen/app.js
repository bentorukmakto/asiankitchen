...
function addMenu(menuGoster) {
  menuDOM.innerHTML = "";
  for (let item in menuGoster) {
    let menuItem = document.createElement("div");
    menuItem.classList.add("menu-items", "col-lg-6", "col-sm-12");
    menuItem.innerHTML = `<img src="${menuGoster[item].img}" alt="${menuGoster[item].title}" class="photo">
    <div class="menu-info">
      <div class="menu-title">
        <h4>${menuGoster[item].title}</h4>
        <h4 class="price">${menuGoster[item].price}</h4>
      </div>
      <div class="menu-text">
      ${menuGoster[item].desc}
      </div>
    </div>`
